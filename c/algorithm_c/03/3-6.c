...
/*
回溯法：
    在包含问题的所有解的解空间树中，按照深度优先搜索的策略，从跟节点出发深度探索解空间树。
    当探索到某一节点时，要先判断该节点是否包含问题的解，如果包含就从该节点出发继续探索下去；
    如果该节点不包含问题的解，那就说明以该节点为根节点的子树一定不包含问题的最终解，因此要跳过对
    以该节点为根节点的子树的系统探索，逐层向其祖先节点回溯。这个过程叫做解空间树的“剪枝”操作。
*/
